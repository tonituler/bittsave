...
class ShareStatementWidget extends StatefulWidget {
  ...
  const ShareStatementWidget(
      {Key key, @required this.amount, @required this.transactionType, @required this.contactModel, @required this.charge, @required this.trxId})
      : super(key: key);
                ...
                children: [
                  ...
                  Center(
                      child: Text('successful'.tr,
                          style:
                              montserratRegular.copyWith(fontSize: Dimensions.FONT_SIZE_OVER_OVER_LARGE, color: ColorResources.getPrimaryColor()))),
                  const SizedBox(
                    height: Dimensions.PADDING_SIZE_DEFAULT,
                  ),
                  Text('${PriceConverter.balanceWithSymbol(balance: widget.amount)}',
                      style: montserratSemiBold.copyWith(fontSize: 40, color: ColorResources.getPrimaryColor())),
                  widget.transactionType != 'request_money'
                      ? Text('charge'.tr + ' ' + widget.charge,
                          style: montserratRegular.copyWith(fontSize: Dimensions.FONT_SIZE_LARGE, color: ColorResources.getPrimaryColor()))
                      : SizedBox.shrink(),
                  const SizedBox(
                    height: Dimensions.PADDING_SIZE_DEFAULT,
                  ),
                  ...
                  Text(
                      widget.transactionType == 'send_money'
                          ...
                          : widget.transactionType == 'cash_out'
                              ? 'cash_out'.tr
                              : 'request_money'.tr,
                      style: montserratSemiBold.copyWith(fontSize: Dimensions.FONT_SIZE_OVER_LARGE)),
                  const SizedBox(height: Dimensions.PADDING_SIZE_SMALL),
                  Center(
                      child: Text('${widget.contactModel.name == null ? '' : widget.contactModel.name} (${widget.contactModel.phoneNumber})',
                          textAlign: TextAlign.center,
                          style: montserratRegular.copyWith(fontSize: Dimensions.FONT_SIZE_DEFAULT, color: ColorResources.getPrimaryColor()))),
                  const SizedBox(height: Dimensions.PADDING_SIZE_SMALL),
                  widget.trxId != null
                      ? Text('trx_id'.tr + ': ${widget.trxId}', style: montserratMedium.copyWith(fontSize: Dimensions.FONT_SIZE_DEFAULT))
                      : SizedBox(),
                  const SizedBox(
                    ...
                    children: [
                      ...
                      Padding(
                        ...
                        child: Text('${splashController.configModel.companyName}',
                            style:
                                montserratMedium.copyWith(fontSize: Dimensions.FONT_SIZE_OVER_OVER_LARGE, color: ColorResources.getPrimaryColor())),
                      ),
                    ],
                  )
                ],
