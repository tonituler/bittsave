...
class BottomSheetWithSlider extends StatefulWidget {
  ...
  const BottomSheetWithSlider(
      {Key key, @required this.amount, @required this.amountCharge, this.pinCode, this.transactionType, this.purpose, this.contactModel})
      : super(key: key);
...
class _BottomSheetWithSliderState extends State<BottomSheetWithSlider> {
  ...
  Widget build(BuildContext context) {
    ...
    double cashOutCharge =
        double.parse(widget.amount.toString()) * (double.parse(Get.find<SplashController>().configModel.cashOutChargePercent.toString()) / 100);
    ...
    return WillPopScope(
      ...
      child: Container(
        ...
        child: GetBuilder<TransactionMoneyController>(builder: (transactionMoneyController) {
          return Column(
                      ...
                      children: [
                        transactionMoneyController.isNextBottomSheet
                            ? Lottie.asset(Images.success_animation,
                                width: Dimensions.SUCCESS_ANIMATION_WIDTH, fit: BoxFit.contain, alignment: Alignment.center)
                            : Padding(
                                padding: const EdgeInsets.all(Dimensions.PADDING_SIZE_SMALL),
                                child: Lottie.asset(Images.failed_animation,
                                    width: Dimensions.FAILED_ANIMATION_WIDTH, fit: BoxFit.contain, alignment: Alignment.center),
                              ),
                      ],
                    )
                  ...
                  children: [
                    !transactionMoneyController.isNextBottomSheet && widget.transactionType != "request_money"
                        ? Row(
                            ...
                            children: [
                              Text('charge'.tr, style: montserratMedium.copyWith(fontSize: Dimensions.FONT_SIZE_DEFAULT)),
                              ...
                              Text(
                                  widget.transactionType == 'send_money'
                                      ...
                                      : PriceConverter.balanceWithSymbol(balance: cashOutCharge.toString()),
                    ...
                    transactionMoneyController.isNextBottomSheet
                        ? Text(
                            ...
                            style: montserratMedium.copyWith(fontSize: Dimensions.FONT_SIZE_LARGE, color: ColorResources.getPrimaryTextColor()))
                        : SizedBox(),
                    SizedBox(height: Dimensions.PADDING_SIZE_EXTRA_EXTRA_LARGE),
                    Text('${PriceConverter.balanceWithSymbol(balance: widget.amount)}', style: montserratMedium.copyWith(fontSize: 34.0)),
                    SizedBox(height: Dimensions.PADDING_SIZE_LARGE),
                    Text(
                        'new_balance'.tr +
                            ' ' +
                            '${widget.transactionType == 'request_money' ? PriceConverter.newBalanceWithCredit(inputBalance: double.parse(widget.amount)) : PriceConverter.newBalanceWithDebit(inputBalance: double.parse(widget.amount), charge: widget.transactionType == 'send_money' ? double.parse(Get.find<SplashController>().configModel.sendMoneyChargeFlat.toString()) : cashOutCharge)}',
                        style: montserratRegular.copyWith(fontSize: Dimensions.FONT_SIZE_DEFAULT)),
                    ...
                    Padding(
                      ...
                      child: Column(
                        children: [
                          Text(widget.transactionType != "cash_out" ? widget.purpose : 'cash_out'.tr,
                              style: montserratRegular.copyWith(fontSize: Dimensions.FONT_SIZE_DEFAULT)),
                          Row(
                            mainAxisAlignment: MainAxisAlignment.center,
                            children: [
                              ...
                              Text(widget.contactModel.phoneNumber, style: montserratSemiBold.copyWith(fontSize: Dimensions.FONT_SIZE_DEFAULT)),
                            ],
                          ),
                          SizedBox(height: Dimensions.PADDING_SIZE_EXTRA_SMALL),
                          transactionMoneyController.isNextBottomSheet
                              ? transactionId != null
                                  ? Text('TrxID: $transactionId', style: montserratLight.copyWith(fontSize: Dimensions.FONT_SIZE_DEFAULT))
                                  : SizedBox()
                              : SizedBox(),
                        ],
                      ),
                      ...
                      children: [
                        ...
                        CustomInkWell(
                            onTap: () async => await Get.find<ScreenShootWidgetController>().statementScreenShootFunction(
                                ...
                                trxId: transactionId),
                            child: Text('share_statement'.tr, style: montserratMedium.copyWith(fontSize: Dimensions.FONT_SIZE_LARGE))),
                        SizedBox(height: Dimensions.PADDING_SIZE_DEFAULT),
                      ],
                    )
                  : SizedBox(),
              transactionMoneyController.isNextBottomSheet
                  ...
                  : transactionMoneyController.isLoading
                      ...
                      : ConfirmationSlider(
                          ...
                          textStyle: montserratRegular.copyWith(fontSize: Dimensions.PADDING_SIZE_LARGE),
                          shadow: BoxShadow(),
                          ...
                          onConfirmation: () async {
                            if (widget.transactionType == "send_money") {
                              ...
                              transactionMoneyController
                                  .cashOutMoney(contactModel: widget.contactModel, amount: double.parse(widget.amount), pinCode: widget.pinCode)
                                  .then((value) {
                                transactionId = value.body['transaction_id'];
                              });
                            }
